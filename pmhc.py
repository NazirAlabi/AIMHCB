    ...
    if not st.session_state.authenticated:
        ...
        with tab1:
            ...
            with st.form("login_form"):
                ...
                if login_button:
                    user_info = authenticate_user(username, password, USER_DB_FILE=USER_DB_FILE)
                    if user_info:
                        ...
                        st.session_state.pp = load_user_profile(username, PROFILE_DIR=None) or ""
            ...
            if st.button("Logout"):
                if not st.session_state.developer:
                    if not st.session_state.first_prompt:
                        ...
                        save_user_profile(username, profile_text, PROFILE_DIR=None)
                st.session_state.authenticated = False
                st.session_state.developer=False
                st.session_state.user_info = None
